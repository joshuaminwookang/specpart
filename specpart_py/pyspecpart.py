...
def _spectral_kmeans(L: "np.ndarray", k: int) -> List[int]:
    import numpy as np
    ...
    # k-means clustering
    centers = coords[:k].copy()
    labels = [0] * coords.shape[0]
    for _ in range(10):
        for i, row in enumerate(coords):
            dists = [np.linalg.norm(row - c) for c in centers]
            labels[i] = int(np.argmin(dists))
        for j in range(k):
            pts = coords[[i for i, l in enumerate(labels) if l == j]]
            if len(pts) > 0:
...
def spectral_partition(graph, nparts: int = 2) -> Tuple[List[int], float]:
    ...
    if nx is not None and hasattr(graph, "edges"):
        ...
        if nparts == 2:
            ...
            parts = _spectral_kmeans(L, nparts)
        cut = 0.0
    ...
    if nparts == 2:
        ...
        group_size = max(1, n // nparts)
        for i, v in enumerate(order):
            parts[v] = min(i // group_size, nparts - 1)
